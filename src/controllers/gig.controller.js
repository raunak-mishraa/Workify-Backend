...
const deleteGig = asyncHandler(async (req, res) => {
   const gig = await Gig.findById(req.params.id)
    ...
    if(gig.userId.toString() !== req.user._id.toString()) return res.status(403).json({message:"You are not authorized to delete this gig"})
    const deleted = await Gig.findByIdAndDelete(req.params.id)
    if(!deleted) return res.status(500).json({message:"An error occured while deleting the gig"})
